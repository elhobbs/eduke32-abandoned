...
//    flushlogwindow = 1;
    ...
    if (g_networkMode != NET_DEDICATED_SERVER)
    {
		//printf("seetting 3DS video mode...\n");
        if (setgamemode(ud.config.ScreenMode,ud.config.ScreenWidth,ud.config.ScreenHeight,ud.config.ScreenBPP) < 0)
        {
            int32_t i = 0;
            int32_t xres[] = {ud.config.ScreenWidth,400,400,400};
            int32_t yres[] = {ud.config.ScreenHeight,240,240,240 };
            int32_t bpp[] = {32,16,8};
		...
		//printf("setgamemode complete\n");

        setbrightness(ud.brightness>>2,g_player[myconnectindex].ps->palette,0);
		//printf("setbrightness complete\n");

        S_MusicStartup();
		//printf("S_MusicStartup complete\n");
		S_SoundStartup();
		//printf("S_SoundStartup complete\n");
	}
//    loadtmb();

    M_Init();
	///printf("M_Init complete\n");
    ...
    FX_StopAllSounds();
	//printf("FX_StopAllSounds complete\n");
	S_ClearSoundLocks();
	//printf("S_ClearSoundLocks complete\n");
...
MAIN_LOOP_RESTART:
    ...
    if (ud.warp_on == 0)
    {
		...
		if ((g_netServer || ud.multimode > 1) && boardfilename[0] != 0)
        {
			...
			//printf("G_NewGame_EnterLevel\n");
			G_NewGame_EnterLevel();

			//printf("Net_WaitForServer\n");
			Net_WaitForServer();
        }
		else if (g_networkMode != NET_DEDICATED_SERVER) {
			//printf("G_DisplayLogo\n");
			G_DisplayLogo();
		}

        if (g_networkMode != NET_DEDICATED_SERVER)
        {
			//printf("G_PlaybackDemo\n");
			if (G_PlaybackDemo())
            {
                FX_StopAllSounds();
                S_ClearSoundLocks();
...
//    ud.auto_run = ud.config.RunMode;
	//printf("P_SetupMiscInputSettings\n");
