...
int32_t xres=-1, yres=-1, bpp=0, fullscreen=1, bytesperline;
...
#if !defined _WIN32 && defined __GNUC__ && !defined __OpenBSD__ && !(defined __APPLE__ && defined __BIG_ENDIAN__) && !defined GEKKO && !defined EDUKE32_TOUCH_DEVICES && !defined __OPENDINGUX__ && !defined _3DS
...
void getvalidmodes(void)
{
    ...
    //printf("Detecting fs video modes: %d\n", sizeof(int));
    ...
    for (i = 0; i < n; i++)
    {
		...
		//printf("%d: %d %d %d\n", (int)i, (int)dispmode.w, (int)dispmode.h, (int)dispmode.format);
	...
	//printf("Detecting wn video modes:\n");
    ...
    for (i = 0; defaultres[i][0]; i++)
    {
		...
		//printf("%d: %d %d\n", (int)i, (int)defaultres[i][0], (int)defaultres[i][1]);
...
{
	//printf("setvideomode_sdlcommon %d %d %d %d\n",*x,*y,c,fs);
    if ((fs == fullscreen) && (*x == xres) && (*y == yres) && (c == bpp) && !videomodereset)
    {
		//printf("OSD_ResizeDisplay %d %d\n", xres, yres);
		OSD_ResizeDisplay(xres, yres);
        return 0;
    }

	//printf("checkvideomode %d %d %d %d\n", *x, *y, c, fs);
	...
	//printf("setvideomode_sdlcommon done\n");
	return 1;
}
...
void copy_screen(u8 *src, int stride, u8* pal) {
	//gfxSetDoubleBuffering(GFX_TOP, false);
	u8* bufAdr = (u8 *)gfxGetFramebuffer(GFX_TOP, GFX_LEFT, NULL, NULL);
	int w, h;
	...
	for (w = 0; w<400; w++)
	{
		for (h = 0; h<240; h++)
		{
			uint32_t v = (w * 240 + h) * 3;
			uint32_t v1 = ((239 - h) * stride + w);
			bufAdr[v] = pal[src[v1] * 4 + 2];
			bufAdr[v+1] = pal[src[v1] * 4 + 1];
			bufAdr[v+2] = pal[src[v1] * 4 + 0];
		}
	}
	gfxSwapBuffers();
}
...
void showframe(int32_t w)
{
    ...
    {
		//printf("----SDL_UpdateTexture--- %08x %d\n", sdl_surface->pixels, sdl_surface->pitch);
		copy_screen((u8*)sdl_surface->pixels, sdl_surface->pitch, (u8*)sdlayer_pal);
		/*FILE *f = fopen("texture.bin", "w");
		fwrite(sdl_surface->pixels, sdl_surface->h, sdl_surface->pitch, f);
		fclose(f);*/
...
static SDL_Scancode buttonMap3ds[] = {
	/* 0*/SDL_SCANCODE_A,
	/* 1*/SDL_SCANCODE_B,
	/* 2*/SDL_SCANCODE_ESCAPE,
	/* 3*/SDL_SCANCODE_RETURN,
	/* 4*/SDL_SCANCODE_RIGHT,
	/* 5*/SDL_SCANCODE_LEFT,
	/* 6*/SDL_SCANCODE_UP,
	/* 7*/SDL_SCANCODE_DOWN,
	/* 8*/SDL_SCANCODE_R,
	...
	/*14*/SDL_SCANCODE_COMMA,
	/*15*/SDL_SCANCODE_PERIOD,
	...
	/* 0*/SDL_SCANCODE_UNKNOWN,
	/* 0*/SDL_SCANCODE_UNKNOWN,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_MOUSE1,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_MOUSE2,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_MOUSE3,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_MOUSE4,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_JOY1,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_JOY2,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_JOY3,
	/* 0*/SDL_SCANCODE_UNKNOWN,//K_JOY4
};
...
int32_t handleevents(void) {
	...
	for (int i = 0; i < 32; i++) {
		if (buttonMap3ds[i]) {
			SDL_Scancode sc = buttonMap3ds[i];
			code = keytranslation[sc];
			if ((down & BIT(i)) != 0) {
